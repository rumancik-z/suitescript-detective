...
// service worker (inline search, including the deadline-checked regex scan).
...
export class Term {
  ...
  constructor(raw, { caseSensitive = false, regex = false, wholeWord = false } = {}) {
    ...
    if (regex || wholeWord) {
      ...
      // Whole-word anchoring is only meaningful for literal terms; wrapping
      // a user regex in \b...\b makes anchored patterns (^foo, foo$) unable
      // to match, so plain regex is left unanchored.
      const src = !regex && wholeWord ? `\\b(?:${base})\\b` : base;
 ...
 *
 * This is what enforces the regex timeout in the service worker: the deadline
 * is checked between files, so a pathological pattern on a single line is the
 * one case it cannot interrupt — the minified/long-line filter excludes those
 * files from regex searches instead.
 *
 * @param {import('./constants.js').SourceRecord[]} sources
 * @param {object} query AST from {@link buildQuery}
 * @param {{cap?: number, sort?: 'relevance'|'file'|'folder'|'line'}} [options]
