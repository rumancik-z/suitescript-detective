 ...
 * @property {string} cursor - last inventory cursor (persisted for future resume support; currently builds always re-paginate from 0)
...
// ---- Search enhancements ----
// Hard timeout (ms) for the deadline-checked inline regex scan (checked
// between files; a single pathological line is excluded via the minified
// filter instead of being interrupted mid-match).
...
export const FULL_DIFF_LINE_CAP = 2000;
...
// First hostname label of a NetSuite account. Account IDs are usually digits
// (1234567) but some carry a letter prefix (td2927492); both may have a
// hyphenated environment suffix (1234567-sb1). Requiring a digit in the ID
// part keeps rejecting non-account hosts (login, www, app). Group 1 is the
// account ID (suffix stripped).
export const ACCOUNT_ID_RE = /^([a-z]*\d[a-z0-9]*)(?:-[a-z0-9]+)?$/i;
