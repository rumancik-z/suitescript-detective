...
/**
 * Remove cached source shards for specific records of an account (e.g. files
 * that vanished from the inventory).
 * @param {string} accountId
 * @param {SourceRecord[]} records
 * @returns {Promise<void>}
 */
export const removeStaleSources = async (accountId, records) => {
  if (!records || !records.length) return;
  const keys = records.map((r) => srcKey(accountId, r.internalId));
...
export const getUsage = async () => {
  ...
  // With unlimitedStorage the base QUOTA_BYTES (10 MB) no longer applies —
  // chrome.storage.local can grow to roughly 2 GB (or remaining disk space,
  // whichever is less) — so evaluate the near-quota ratio against that
  // effective cap instead of infinity (which would make the warning dead).
