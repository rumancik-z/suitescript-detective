...
export const diffLines = (linesA, linesB) => {
  ...
  let processed = 0; // ops consumed by pre-hunk prefix + earlier hunks
  for (const { opsStart, opsEnd } of hunkRanges) {
    // Advance counters across ops not yet consumed (the pre-hunk prefix or
    // the inter-hunk gap — both contain only "equal" ops, but count via the
    // tag checks so this stays correct if that ever changes).
    for (let k = processed; k < opsStart; k++) {
