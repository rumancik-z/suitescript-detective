...
// (background.js). Regex searches therefore run through the cooperative
// `scanSourcesAsync`, which yields between sources and aborts once
// REGEX_TIMEOUT_MS is exceeded. This guards the common cases (many sources /
// many lines); a pathological pattern on a single very long line cannot be
// interrupted in-process, but minified/bundled records (which carry the very
// long lines) are always excluded from regex searches and the `regexMaxLines`
// setting caps file size as well.
