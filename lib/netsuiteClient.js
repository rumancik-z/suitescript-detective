...
// Hard ceiling on a single network request. Without it, one hung fetch
// (NetSuite can do this) leaves the build promise unsettled forever and the
// worker's build slot claimed until the worker is recycled.
const FETCH_TIMEOUT_MS = 60000;

/**
 * fetch() with a hard timeout; the request is aborted (AbortError) when the
 * deadline passes.
 * @param {string} url
...
export const fetchInventoryPage = async (account, cursor, folders = SCRIPT_FOLDERS) => {
  ...
  try {
    ...
    // No retry loop here — a timed-out inventory page should fail the build
    // with a clear message instead of hanging the worker's build slot.
    if (err && err.name === "AbortError") throw new Error("Inventory request timed out — check your connection and try again.");
    throw err;
  }
...
/**
 * Pull a printable string out of a column cell. Handles scalars as well as
 * nested object shapes (e.g. {value: {text: "..."}}) by recursing, so a
 * nested cell no longer stringifies to "[object Object]".
 * @param {unknown} cell
 * @returns {string}
 */
const cellText = (cell) => {
  ...
  if (typeof cell === "object") {
    if (cell.text != null && cell.text !== "") return cellText(cell.text);
    if (cell.value != null) return cellText(cell.value);
    if (cell.name != null) return cellText(cell.name);
  }
  return "";
};

/**
 * Pull the underlying id/value out of a column cell. Recurses into nested
 * object shapes (e.g. {value: {text: "..."}}) instead of stringifying them.
 * @param {unknown} cell
 * @returns {string}
 */
const cellValue = (cell) => {
  ...
  if (typeof cell === "string" || typeof cell === "number") return String(cell);
  if (typeof cell === "object") {
    if (cell.value != null) return cellValue(cell.value);
    if (cell.internalid != null) return cellValue(cell.internalid);
 ...
 * Also recurses into nested object shapes (e.g. {value: {text: "/c.123/…"}}).
 * @param {unknown} cell
 * @returns {string}
 */
const cellUrl = (cell) => {
  ...
  if (typeof cell === "object") {
    // First pass: direct string values; second pass: recurse into nested
    // objects (e.g. {value: {text: "/c.123/app/x.js"}}).
    for (const key of ["value", "url", "href", "text", "name"]) {
      const v = cell[key];
      if (typeof v === "string" && looksLikeFileUrl(v)) return v;
    }
    for (const key of ["value", "url", "href", "text", "name"]) {
      const nested = cellUrl(cell[key]);
...
export const fetchSource = async (account, entry) => {
  ...
  while (true) {
    ...
    try {
      ...
      // A timed-out request aborts as an AbortError — normalize it to a
      // transient error so it rides the existing retry/backoff path below
      // (rethrowing from this catch would escape the retry loop entirely).
      if (err && err.name === "AbortError") err = new Error("transient timeout");
...
export const toSourceRecord = (entry, text) => {
  // Normalize CRLF → LF before splitting so stored lines never carry a
  // trailing \r (breaks $-anchored regexes, spurious CRLF/LF diffs).
